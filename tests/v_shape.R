#assess the SpODT algorithm to detect a "V" border shape situation: Clustered data with higher values under a "V" border


require(SPODT)
require(tree)

data(dataV0) #no cluster
data(dataV0_5) #cluster with moderate values
data(dataV1_5) #cluster with high values
data(dataV2) #cluster with very high values

beta<-c(0,0.5,1.5,2)

dataset <- vector('list', 4)
dataset[[1]]$dat<-dataV0
dataset[[2]]$dat<-dataV0_5
dataset[[3]]$dat<-dataV1_5
dataset[[4]]$dat<-dataV2
